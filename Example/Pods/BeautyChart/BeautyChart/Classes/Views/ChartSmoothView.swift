...
public struct ChartSmoothView: View{
    ...
    @ObservedObject var pressPosition:PressPositionWrapper = PressPositionWrapper()
    
    var title:String = "Smooth points line"
    var caption:String = "Side information"
    ...
    public init(points:[CGPoint], style:LineViewStyle = LineViewStyle()){
    ...
    func pathDrawing(_ geo:GeometryProxy, forFill:Bool=false) -> Path{
        ...
        if !self.styleAdaptive.bezierStepMode{
            ...
            for i in 0..<(drawPoints.count-1){
                ...
                var tmpContrPoint = BezierSegmentControlPoints(firstControlPoint: fPoint, secondControlPoint: sPoint)
        ...
        if forFill{
            let cachedLimits = self.points.limits
    ...
    public var body: some View{
            ...
            VStack(alignment: .leading){
                ...
                ZStack{
                    GeometryReader {
                            ...
                            ZStack {
                                Group {
                                    ...
                                    self.pathDrawing(geo).trim(from: 0, to: self.percentage) // << breaks path by parts, animatable
                                        ...
                                        self.mainPath = self.pathDrawing(geo)
                                    }
                                ...
                                if (self.pressVisible) {
                                    PressLineView(position: self.pressPosition, style: self.styleAdaptive)
                                }
                            }
                        ...
                        .onChanged({ value in
                            self.mainPath = self.pathDrawing(geo)
                            self.pressPosition.coordinates = geo
                            self.pressPosition.pointPosition = self.mainPath!.point(for: value.location.x/geo.size.width)
                            self.pressPosition.update(value.location)
                            if value.location.x <= geo.size.width && value.location.x >= 0{
                                self.pressVisible = true
                                let revPoint =  self.points.reverseAffine(self.pressPosition.pointPosition, width: Float(geo.size.width), height: Float(geo.size.height), vMirrored: true)
                                let closest =  self.points.closestPoint(revPoint, axes: self.styleAdaptive.movingFrameCloserPointMethod)
                                if self.pressPosition.text !=  String(Float(closest.y)){
                                    HapticFeedback.playSelection()
                                }
                                self.pressPosition.text =  String(Float(closest.y))
        ...
        .cornerRadius(10).if(self.styleAdaptive.dropShadow){
            $0.shadow(color: .gray, radius: 8, x: 0, y: 0)
        }
    .animation(nil)
...
class PressPositionWrapper: ObservableObject{
    @Published var position: CGPoint = CGPoint(x:0, y:0)
    @Published var pointPosition: CGPoint = CGPoint(x:0, y:0)
    @Published var text: String = "as"
    var coordinates:GeometryProxy?
    
    init(_ geo:GeometryProxy){
        self.coordinates = geo
    }
    init(){
