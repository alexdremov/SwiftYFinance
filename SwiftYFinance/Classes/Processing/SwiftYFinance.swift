...
public class SwiftYFinance{
    ...
    */
    ...
    /**
    Fetches a lot of information about company. Check` IdentifierSummary` to find what fields are available
     - Warning: Identifier must exist or data will be nil and error will be setten
     - Parameters:
         - identifier: Name of identifier
     */
    public class func getBigSummaryOfEquityBy(identifier:String, callback: @escaping (IdentifierSummary?, Error?)->Void){
        ...
        AF.request(requestURL!).responseString(queue:DispatchQueue.global(qos: .utility)){ response in
            ...
            callback(IdentifierSummary(information: jsonSummary), nil)
        }
    }
    
    /**
    The same as `SwiftYFinance.getBigSummaryOfEquityBy(...)` except that it executes synchronously and returns data rather than giving it to the callback.
    */
    public class func syncGetBigSummaryOfEquityBy(identifier:String) -> (IdentifierSummary?, Error?){
